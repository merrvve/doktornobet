...
export class HomeComponent {
  ...
  setPersons() {
    ...
    const selected = new Set(
      this.doctorTypes.filter((x) => x.checked == true).map((item) => item.id),
    );
  ...
  hesapla() {
    ...
    for (let i = 1; i < this.personsList.length; i++) {
      dayslocal = [];
      let possibleIds =
        this.personsList[i].length < 0
          ? []
          : this.createNumberList(0, this.personsList[i].length - 1);
    ...
    for (let j = 1; j < this.personsList.length; j++) {
      ...
      this.dayslist[j] = this.shiftService.nobetleriHesapla(
        this.dayslist[j],
        this.personsList[j],
        holidays,
      );
