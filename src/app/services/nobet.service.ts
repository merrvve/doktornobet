...
export class NobetService {
  ...
  markNotPossibleDays(persons: Person[], days: Day[]) {
    for (const person of persons) {
      for (const busyDay of person.notPossible) {
        const index = days[busyDay - 1].possiblePersonIds.indexOf(
          person.personId,
        );
        if (index > -1) {
  ...
  assignLowPossibilityDays(
    days: Day[],
    persons: Person[],
    shiftPerPerson: number,
    holidayShiftPerPerson: number,
    ...
    for (const day of days) {
      if (
        day.possiblePersonIds.length == 0 &&
        (day.workingPersonIds.length == 0 || day.workingPersonIds.length == 1)
      ) {
        console.log('No one is possible for day: ', day.dayNo);
        return;
      } else if (
        day.possiblePersonIds.length == possibleNumber &&
        (day.workingPersonIds.length == 0 || day.workingPersonIds.length == 1)
      ) {
        let selectedPersonId = this.sample(day.possiblePersonIds);
        day.workingPersonIds.push(selectedPersonId);
        const selectedPerson = persons.find(
          (x) => x.personId == selectedPersonId,
        );
        if (selectedPerson) {
          this.assignPersonToDay(
            selectedPerson,
            days,
            day,
            shiftPerPerson,
            holidayShiftPerPerson,
            finishedPersonsForHolidays,
            finishedPersons,
          );
        }
      }
  ...
  assignPersonToDay(
    ...
    if (person) {
      ...
      if (dayId != 0) {
        days[dayId - 1].possiblePersonIds.splice(
          days[dayId - 1].possiblePersonIds.indexOf(person.personId),
          1,
        );
      }
  ...
  nobetleriHesapla(days: Day[], persons: Person[], holidays: number[]) {
    console.log(days);
    const shiftPerPerson = Math.trunc(
      (days.length - holidays.length) / persons.length,
    );
        ...
        if (selectedId > -1) {
          ...
          if (selectedPerson) {
            if (days[i].isHoliday) {
              ...
              if (
                selectedPerson.holidayShiftDays.length >= holidayShiftPerPerson
              ) {
